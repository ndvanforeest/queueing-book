...
In other words, if the person does not tell us anything, we write the the probability that no arrival occurs before~$t$ as $\P{X>t}$.
...
\end{equation}
A rv~$X$ whose cdf satisfies this property is said to be \recall{memoryless}.
...
If we have a sequence of interarrival times $\{X_{i}\}_{i=1}^{\infty}$, then we construct job (customer, patient) arrival times according to the rule $A_{k} = A_{k-1} + X_{k}$, starting at $A_{0} = 0$.
When interarrival times are iid rvs and distributed as the common rv $X\sim \Exp{\lambda}$, it is clear that the density of the first arrival time is
\begin{equation*}
...
\begin{align}
\P{N( t) = 0} &= e^{-\lambda t} = 1-\lambda  t + o( t),  \label{eq:4}\\
...
Then the process of customers entering the shop is a Poisson process $N_{\lambda p}$ with rate $\lambda p$, because, from~\eqref{eq:32}, for $t\ll 1$,
\begin{align*}
