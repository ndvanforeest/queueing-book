...
With the recursions of the previous section we can simulate simple systems.
...
The relating code is simple, but there are two small details we should solve.
First, suppose we specify the inter-arrival times~$X$ as a numpy vector, and we compute the arrival times~$A$ as the \pyv{cumsum} of~$X$.
\begin{python}
import numpy as np
...
print(A)
\end{python}
So \pyv{A = [2, 5, 9]}, but this is not what we want, because according to~\cref{eq:qc-4} the arrival times should start with $A_{0}=0$.
Moreover, $A_{1} = A_0 + X_{1}$ so we never use $X_{0}$.
The solution is to set \pyv{X[0]=0}, and then take the \pyv{cumsum}.
The second problem is when we generate a number of service times as a numpy vector.
If we were to compute the total offered amount of work, then the natural thing is to call \pyv{S.sum()}.
However, this includes \pyv{S[0]}, while actually job~$0$ is never served.
So, we set \pyv{S[0]=0} too. This induces a slight error though: \pyv{S.mean()} includes still $S[0]$, so we divide by one job too much; to repair we should compute \pyv{S.mean() * len(S) / (len(S) -1)}, but we choose to neglect this small error.
...
Before discussing how to get around this, we first show how to compute the time-average of the number of jobs in the system, as this is still relatively easy and develops some formulas that we will need for the proof of Little's law, c.f., \cref{sec:littles-law}.

\cref{fig:atltdt} clarifies that the number of jobs $\L(t)$ in the system at time~$t$ is equal to all jobs $A(t)$ that arrived minus the jobs $D(t)$ that departed during $[0,t]$, assuming that $L(0)= 0$.
Hence, the mean number of jobs during $[0,t]$ must be $t^{-1}\int_0^t \L(s) \d s$.
Another way to look at the integrand $\L(s)$ is to count all jobs whose arrival times lie before~$s$ and whose departure time lie after~$s$, that is, $\L(s) = \sum_{k=1}^{n} \1{A_k\leq s \leq D_{k}}$, where we assume that the simulation involves~$n$ jobs in total. Therefore, the time-average number of~$L$ is
\begin{align*}
  \frac{1}{t}\int_{0}^{t} L(s) \d s =
  \frac{1}{t}\int_{0}^{t} \sum_{k=1}^n \1{A_k\leq s \leq D_{k}}) \d s =
  \frac{1}{t} \sum_{k=1}^n \int_{0}^{t}\1{A_k\leq s \leq D_{k}}) \d s =
  \frac{1}{t} \sum_{k=1}^n J_{k},
\end{align*}
because the integral $\int_{0}^{t} \1{A_k\leq s \leq D_k} \d s = J_{k}$ if $D_k \leq t$.
...
print(heappop(heap)) # Why is the dog?
print(heappop(heap)) # Why is the cat?
print("Add Elephant")
heappush(heap, (23, "Elephant"))
...
For this to work, we form events of type $(t, \Delta)$ where~$t$ is arrival or departure time and $\Delta \in \{-1, 1\}$ to correspond to a departure or arrival, respectively.
To sort the events, we form two lists of events and merge them.

There is technical, but important, detail involved.
The function \pyv{heapq.merge} returns a generator, not a list, but \mintinline{python}{heapq.heapify} expects a list instead of a generator.
To convert the generator obtained from \mintinline{python}{heapq.merge} we apply \pyv{list} to this generator, and then pass the resulting list to \pyv{heapq.heapify}.
\begin{python}
...
With this heap we can already plot the system length, but for the virtual waiting time need some additional code.
Recall from~\cref{ex:l-120} that $V(t) = [J_{A(t)} - (t-A_{A(t)})]^{+}$, and that $A(t) = \sum_{k=1}^{\infty} \1{A_k\leq t}$. The following code would work.
\begin{python}
k = 1
while A[k] <= t:
    k += 1
...
Instead, we should stick the rule that says `Thou shalt use binary search to find an element in a sorted sequence', we use \pyv{np.searchsorted} to efficiently find the index~$k$ in an array~$A$ such that $A_{k-1} \leq t < A_{k}$ for some given value~$t$.
...
\end{python}
Note that we include the option \pyv{side} to provide us with the left-continuous version $A(t-)$; $A(t)$ is defined as right-continuous.
...
All is in place to make~\cref{fig:virtual_sojourn}. We have two panels \pyv{ax1} and \pyv{ax2} on top of each other, and we set the labels at the axes.
\begin{python}
...
\end{python}
Setting the opacity \pyv{alpha} to $0.2$  gives nice  results for overlapping triangles.


Next is the code to plot~$L$ and~$V$. Pay attention to the fact that we use $V(t-)$ (in code \pyv{V(now, 'left')}). Lines~$5$ and~$7$ plot vertical dotted lines for visual clarity. Formally speaking, these lines do not belong to the virtual waiting time or system length processes.
\begin{python}
L, past = 0, 0
while heap:
