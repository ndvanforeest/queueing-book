...
In this section we devevelop many different queueing models by making judicious choices for $\lambda(n)$, $\tilde \lambda(n)$ and $\mu(n)$ in the level-crossing equations~\cref{eq:25}.
It is much more important to understand how to specify the correct level-crossing equations than to derive theexpressions for $\E L$, say; the main text therefore only presents the results, and delagates the derivations to  the exercises.
We first discuss models for which we can obtain closed-form solutions for the performance measures.
Then we sketch an algorithm to compute these measures for the general case.
We assume that all systems that do not block arrivals in some way are stable; when demand is blocked, the systems is stable anyway.


\newthought{In the models} below we allow $\lambda(n)$ and $\mu(n)$ to depend on the number~$n$ in the system, but given~$n$, the inter-arrival and service times should be exponentially distributed, that is,\sidenote{This very general queueing systems is sometimes written as the $M(n)/M(n)/1$ queue.}
\begin{align*}
  X|L=n &\sim \Exp{\lambda(n)}, & S|L=n &\sim \Exp{\mu(n)}.
\end{align*}
However, as in the general case arrivals need not enter, we consider
\begin{align*}
...
\newthought{The \recall{$M/M/1$} queue} has  a constant arrival and service rate so that we take
\begin{align*}
\lambda(n)&=\lambda, & \mu(n)&=\mu.
\end{align*}
With this, level-crossing equations become
\begin{equation*}
...
\newthought{The \recall{$M/M/c$} queue} has~$c$ identical servers to process jobs.
To model this, set
\begin{align*}
...
p(n) &= \frac 1 {G} \frac{1}{\prod_{k=1}^{n}\min\{c, k\}}(c\rho)^{n} , \quad n\geq 1,\label{eq:1}\\
...
\end{align}
\end{subequations}
Note in particular that  $\E{\Ls} \neq \rho$.


\newthought{The \recall{$M/M/1/K$} queue} blocks jobs when $L\geq K$.
...
Using $\tilde \lambda(n) p(n) = \mu(n+1)p(n+1)$, it follows right away that
 \begin{align*}
...
The rate at which jobs arrive at the system is $\lambda (K) = \lambda$ because jobs still arrive as a Poisson process, but the rate at which the jobs enter is $\tilde \lambda(K). = 0$ Consequently,
\begin{equation*}
...
Moreover, since jobs are blocked, the utilization is no longer equal to the load; the utilization is now $\lambda (1-p(K))/\mu \neq \lambda /\mu = \rho$, and
\begin{equation}\label{eq:mm1-2}
\E\Ls = \sum_{n=0}^K \min\{n, 1\} p(n) = 1-p(0) = \frac{\lambda}{\mu} (1-p(K)).
\end{equation}

\newthought{The \recall{$M/M/c/c$} queue} blocks jobs when all of its~$c$ servers are occupied.\sidenote{This queueing model is also known as the Erlang~$B$ model.}
Thus, we take
\begin{align*}
...
Hence it seems reasonable to model the recovery times as exponentially distributed.

Just as for the $M/M/c$ queue, $\rho = \lambda/(c \mu)$.  With some algebra\sidenote{\cref{ex:l-245}} we find
 \begin{align*}
...
\end{align*}
The last two results are easy. Jobs either find a free server or are blocked, hence, jobs cannot be in queue.
...
Any job that arrives finds a free server, hence, its service  starts directly aft the arrival of a job.
Therefore,
\begin{align*}
...
\end{align*}
as jobs are not lost, $\tilde \lambda(n) = \lambda(n)$.
...
A simple example model with customer balking is to take,
 \begin{align*}
...
\newthought{The  \recall{$M/M/c/K$} queue} is a multiserver queue with blocking and is a simple generalization of the $M/M/c$ and $M/M/1/K$ queue.
Now,
\begin{align*}
\tilde \lambda(n) &= \lambda\1{n<K}, &\mu(n) &= \mu \min\{n, c\}.
\end{align*}
We can use the code below if we need to analyze such as system.

\newthought{The most general} case can only be approached with numerical methods.
We present example code to deal with a queue with balking; the other models of this section are easy modifications of this.
