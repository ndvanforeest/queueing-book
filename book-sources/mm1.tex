...
It is much more important to understand how to specify the correct level-crossing equations than to derive closed form expressions for performance measures.
...
In the models below we allow $\lambda(n)$ and $\mu(n)$ to depend on the number~$n$ in the system.
However, given~$n$, the inter-arrival and service times should be exponentially distributed, that is,\sidenote{This very general queueing systems is sometimes written as the $M(n)/M(n)/1$ queue.}
\begin{align*}
...
\end{align*}
It is possible\sidenote{\cref{ex:7}} to derive closed form expressions for $\E \Q$ and $p(n)$.
I don't find them really useful.
...
However, two measures are simple:
\begin{align*}
...
Moreover, since jobs are blocked, the utilization is no longer equal to the load; the utilization is now
\begin{equation*}
\lambda (1-p(K))/\mu \neq \lambda /\mu = \rho,
\end{equation*}
and
\begin{equation}\label{eq:mm1-2}
\E\Ls = 1-p(0) = \frac{\lambda}{\mu} (1-p(K)).
\end{equation}
...
\end{align*}
The normalization constant follows  as $G = \sum_{n=0}^{c}p(n)$. As there are as many servers as jobs and, jobs get rejected at rate $1-p(c)$, by PASTA,
 \begin{align*}
   \E \Q &= 0, & \E \Ls &= \frac{\lambda}{\mu} \left(1- p(c)\right).
\end{align*}
...
Any job that arrives finds a free server, hence, its service  starts directly after the arrival of a job.
Therefore,
\begin{align*}
\lambda(n)&=\lambda, & \mu(n) &= n \mu.
\end{align*}
By taking the limit $c\to \infty$ in the expressions of the $M/M/c/c$ queue (or the $M/M/c/c$ queue) we get
\begin{align*}
