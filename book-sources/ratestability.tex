...
\newthought{We first formalize} the arrival rate and departure rate in terms of the arrival and departure processes $\{A(t)\}$ and $\{D(t)\}$, c.f., \cref{sec:constr-gg1-queu}.
...
\end{equation}
Henceforth, we assume that both limits exist, are finite, and $0<\lambda<\infty$.
...
We prove below that our assumptions imply that the average inter-arrival time $\E X$ between two consecutive jobs is equal to the \emph{inverse} of the arrival rate $\lambda$, i.e.,
\begin{equation*}
...
\newthought{When the station} contains multiple identical servers, or arrivals occur in batches and/or are served in batches, the work per job arrival is $\E S \E B$.
Similarly, when a server serves $\E C$ jobs per service time, its service rate is $\E C/\E S$. Finally, when the station contains~$c$ servers, the station's capacity is~$c$ times are large as that of a single server. Thus, in the most general case,
\begin{equation*}
...
\newthought{It remains to} establish that
\begin{equation}\label{eq:limits}
\lim_{t\to\infty} \frac t {A(t)} = \lim_{n\to\infty} \frac {A_{n}} n.
\end{equation}
This equality is not entirely evident because the first limit runs of over all real numbers, while the second runs over the natural numbers.

To show the equality, observe first that the counting funciton $A(t) \to \infty$ as $t\to\infty$ by our assumptions $0<\lambda<\infty$ and the existence of the limit of $A(t)/t\to\lambda$ as $t\to\infty$.
Observe next that $A_{A(t)}$ is the arrival time of the last job \emph{before} time~$t$, hence $A_{A(t)} \leq t$.
This, in turn, implies that $A_{A(t)+1}$ is the arrival time of the first job \emph{after} time~$t$.
Therefore, $A_{A(t)} \leq t < A_{A(t)+1}$, which is equivalent to
\begin{equation*}
 \frac{A_{A(t)}} {A(t)} \leq \frac{t}{A(t)} <\frac{A_{A(t)+1}}{A(t)} = \frac{A_{A(t)+1}}{A(t)+1}\frac{A(t)+1}{A(t)}.
 \end{equation*}
With regard to the left most term, observe that when $A(t)= n$ then\sidenote{\cref{ex:61}} $A_{A(t)} = A_{n}$, so that $A_{A(t)}/A(t) = A_n/n$.
Since $A(t)\to\infty$
 \begin{align*}
\lim_{t\to\infty} \frac{A_{A(t)}}{A(t)} &= \lim_{t\to\infty} \frac{A_{A(t)+1}}{A(t)+1} =  \lim_{n\to\infty} \frac{A_n}{n},\\
\lim_{t\to\infty} \frac{A(t)+1}{A(t)} &=1.
 \end{align*}
This proves~\cref{eq:limits} because the limit in the middle, whose existence we assumed, is wedged between two limits that are the same.

Here is a first idea on how we can use the memoryless property of the inter-arrival times of the $M/G/1$ queue to find show that the expected busy time of the $M/G/1$ satisfies $\E \U = \E S/(1-\rho)$.
