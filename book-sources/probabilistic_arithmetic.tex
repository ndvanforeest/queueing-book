...
We also load the \texttt{typing} module because our code below adds typing information, which means that we specify what type of argument (such as float, or int) is used.\sidenote{In simple scripts I tend not to add typing. However in code that I plan to use later at other places I find this extra information helpful.}
\begin{minted}[]{python}
import operator
from fractions import Fraction
from collections import defaultdict
...
\begin{solution}
Type on the Google prompt: `realpython defaultdict, click on the link that appears.  Then read and think hard. Recall, coding needs an intellectual effort.
\end{solution}
\end{exercise}
\end{document}
