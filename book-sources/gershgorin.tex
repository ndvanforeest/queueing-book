...
%                    \end{pmatrix},
% \end{align*}
% and the latter as $\lambda = \gamma + \lambda P$.\marginnote{A similar form can be found for~\cref{eq:98}.}
...
% Rather than dealing with the ensuing technical complications, we bypass them by restricting the analysis to an $M/M/1/K$ queue under an~$N$-policy, with $K\gg N$ for any~$N$ that can be `reasonable'.\marginnote{What is reasonable depends on the context of course, but $N=1000$ seems ridiculously large for any practical queueing system.}
...
Since~$P$ is a non-negative matrix, $P^n$ is also non-negative for any~$n$, hence, $\sum_{m=0}^n P^m$ is a monotone increasing sequence (of matrices).\marginnote{Formally, non-decreasing sequence.}
...
Using this, it is not hard to prove that $P^n \leq  (1-\epsilon)^n$ when~$P$ is transient.\marginnote{\cref{ex:g-3}}

\newthought{For the second} method, let us make the simplifying assumption that~$P$ is a diagonalizable matrix with~$M$ different eigenvalues.\marginnote{The argument below applies just as well to matrices reduced to Jordan normal form, but adds only notational clutter.}
...
Next, assume for notational simplicity that for each row~$i$ of~$P$ we have that $\sum_{j} p_{i j}<1$.\marginnote{Otherwise apply the argument to $P^M$.}
