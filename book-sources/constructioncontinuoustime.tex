...
Recalling that in a queueing system, a job can either be in queue or in service, we distinguish between the number of jobs in the system $\L(t)$, the number of jobs in queue $\Q(t)$, and the number in service $\Ls(t)$.
Clearly, $\L(t) = \Q(t) + \Ls(t)$.  Sometimes we write  $\tilde A(t)$ for the number of jobs that departed from the queue up to time~$t$; it follows that $\Ls(t) = \tilde A(t) - D(t) = \L(t) - \Q(t)$.
