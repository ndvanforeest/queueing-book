...
Then we can compute the sequence of departure times $\{D_{k}\}$ from the recursion\sidenote{Note that we assume that job~$k$ `reveals' its service time at the moment it arrives.}
\begin{equation}\label{eq:qc-3}
D_k = \max\{A_k, D_{k-1}\} + S_k.
\end{equation}
Why is this true?
The service of job~$k$ cannot start before it arrives, hence, the job cannot leave before $A_k + S_k$ which is the same as saying that $D_k \geq A_k +S_k$.
Moreover, the service of job~$k$ cannot start before job $k-1$ left the server. Thus job~$k$ can also not start before $D_{k-1}$.

Once $A_{k}$ and $D_{k}$ are known, we can compute the \recall{sojourn time} and \recall{waiting time} as
\begin{align*}
\J_k &= D_{k} - A_k, &   &  \W_k = J_k - S_k,
 \end{align*}
thus, $\J_{k}$ is the total amount of time job~$k$ spends in the system, and $\W_{k}$ is the time the job spends in queue (but not at the server).  \cref{fig:waitingtimegg1} shows how the above concepts relate to each other.
...
If we only need  $\J_k$ then the next rule suffices:
\begin{equation}
\label{eq:59}
\J_{k} = [\J_{k-1} - X_k]^+ + S_k.
\end{equation}
...
Specifically,\sidenote{~\cref{ex:60}}
\begin{equation*}
...
Likewise, for the departures $\{D_{k}\}$ and the \emph{departure process} $\{D(t)\}$ we have the relations
\begin{align*}
 D(t) &= \max\{k : D_k \leq t\} = \sum_{k=1}^\infty \1{D_k \leq t}, &  D_{k} &=\min\{t: D(t) \geq k\}.
\end{align*}
...
Clearly, $\L(t) = \QQ(t) + \Ls(t)$.  Sometimes we write  $\tilde A(t)$ for the number of jobs that departed from the queue up to time~$t$; it follows that $\Ls(t) = \tilde A(t) - D(t) = \L(t) - \QQ(t)$.
...
Once at $y=0$, the virtual waiting time remains there until an arrival occurs, c.f., \cref{fig:waitingtimegg1}.
Here is the expression\sidenote{See~\cref{ex:l-150}.}
 \begin{equation}\label{eq:34}
 V(t)= [\J_{A(t)} - (t-A_{A(t)})]^+.
 \end{equation}

 \newthought{\cref{fig:inv_queue} explains graphically} the relation between between a pro\-duc\-tion-inventory and queueing systems.
When a job arrives in the queueing system, the virtual workload~$V(t)$ increases by the service time of the job.
In production-inventory system, a machine replenishes the inventory at some finite speed.
When a customer arrives, the inventory $I(t)$ decreases by the demand size of the customer.
Like this, customer demands in the inventory system are service time of a job in a queueing system.
...
Thus, even though in the inventory system, customers do not have to wait, their demands spawn production times at the machine (a server) who replenishes the consumed items.
...
Thus, in general terms, in a queueing system or an inventory system, there is always `something' or `somebody' waiting.
Items in a supermarket are produced ahead of the moment they are `consumed', hence they `wait' for customers.
In a queueing system, customers are waiting while their product is `produced' by the server.
When there are no customers in a queueing system, the server waits until a new customers comes along.
Thus, queueing and inventory theory are both focused on the distribution of waiting times, either by customers, servers, or items, hence both are related branches of (applied) probability theory and stochastic operations research.
...
\begin{exercise}\label{ex:61}
 What are the meanings of $A_{A(t)}$, and of $A(A_n)$?
\begin{solution}
...
Can $A(t)$ be defined as $\min\{k : A_k \geq t\}$ or as $\min\{k: A_k > t\}$?
\begin{hint}
