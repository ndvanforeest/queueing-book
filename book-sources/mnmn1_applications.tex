...
\end{hint}
\begin{solution}
As people arrive without making appointments, it is reasonable to say that the interarrival times are memoryless.
...
\begin{exercise} The smallest barber shop in town works alone and has two chairs for waiting customers.
 ...
 Then, estimate $\E{\Q}$ when the barber hires another location that allows for 3 extra chairs.
...
\pyv{labda = np.array([10.0, 10.0, 20.0, 20.0, 5.0])}.
...
% Consider the arrival of taxi~$k$. Let this taxi see $\Q_{s,k}$  riders at the head of the line. Let~$b$ be the index of the first group in the queue, hence group $b-1$ stands at the head of the line. Then,
% \begin{align*}
% d_k &= \min\{\Q_{s,k}, 4\}, &\text{riders served by taxi~$k$}, \\
% \Q_{s}' &= \Q_{s,k} - d_k, &\text{riders remaining behind at head of line}, \\
% \Q_k' &= \Q_k + a_{k+1}, &\text{groups waiting just before arrival taxi $k+1$}, \\
...
% \Q_{s, k+1} &=  h_k B_b& \text{riders at head of the line seen by taxi $k+1$}, \\
