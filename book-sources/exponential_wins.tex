...
The \mintinline{python}{fig} object can be thought of as a canvas on which we draw the plots, and \mintinline{python}{axes} is a list of \mintinline{python}{ax} objects, each representing a single plot within the figure.
...
(There is a subtle, small problem when $\P{X=C-1} = \P{X=C} = 1/2$ and the cycle length is $10$.
...
If $\{X_{k}\}_{k=1}^{\infty}$ is a set of iid rvs such that $\E{X_{k}} < \infty$ and $\sup_{i}\P{X_{k} =i} < 1$, then, for all $i$, $\alpha_k(i) \to 1/C$ as $k\to\infty$.
\end{theorem}
...
Interestingly, the ideas of the proofs for~\cref{thr:2} are used in potential theory and complex analysis, and the limiting function distribution is known as a harmonic function.
...
For this purpose I build the next function that overloads the modulo operation to handle ints and floats in the regular way, but treats \texttt{RV} in a specific way, cf., the next section.

\inputminted[firstline=8, lastline=18]{python}{../code/exp_convergence.py} % modfunction

We next compute the position iteratively $\sum_{i=1}^{n} S_{i} \bmod C$ where the step size is $S\sim\Unif{\{-1, 0, 1\}}$.
The position starts at $0$. Oberve that the \texttt{+} operator is overloaded so that it can add two objects of type \texttt{RV}. We keep track of the position with the smallest and the largest probability.
...
\caption{Convergence speed of the time between two visits to the hostipal.
