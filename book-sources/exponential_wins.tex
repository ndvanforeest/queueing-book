...
Each inter-arrival time is $\Unif{[0.9, 1.1]}$.
...
\newthought{It remains to} provide proofs for the claims we make above.
...
Consider iid rvs $\{U_k\}_{k=1}^{\infty}$ such that $U_{k}\sim \Unif{[0,1]}$, and write the order statistic of $\{U_k\}_{i=1}^{n}$ as $0<Y^{n}_1 < Y^n_2 < \cdots < Y_n^{n}<1$.
...
By taking $Y_0^{n} = 0, Y_{n+1}^n=1$, we define the size of the gaps as seen by the hospital as $X_{k}^{n} = Y_k^n - Y_{k-1}^{n}$, $k=1, \ldots, n+1$.
...
\begin{lemma}
\begin{equation*}
\lim_{n\to\infty} \P{n X^n_1 \leq x} = \lim_{n\to\infty} \P{n Y^n_1 \leq x} =1- e^{-x}.
\end{equation*}
\end{lemma}
\begin{proof}
...
Finally, from the definition, it is obvious that $X_1^n = Y_1^n - Y_0^{n} = Y_1^{n}$.
\end{proof}
...
\end{lemma}
\begin{proof}
The probability of the event $Y_1^n < Y_2^n < \cdots < Y_{k-2}^n < Y_{k-1}^n \in [x, x+ \d x] < Y_k^n \in [y, y+\d y] < Y_{k+1}^{n} < \cdots Y_n^{n}$ is given
\begin{align*}
  f_{Y_{k-1}^n, Y_k^n}(x, y) \d x \d y = \frac{n!}{(k-2)!(n-k)!} x^{k-2} (1-y)^{n-k} \d x \d y.
\end{align*}
With this, the probability that $X_k^{n} \in [r, r+\d r]$ becomes
\begin{align*}
  f_{X_{k}^n}(r) \d r = \d r \int_{0}^{1-r}  f_{Y_{k-1}^n, Y_k^n}(x, x+r) \d x.
\end{align*}
Substitute the expression for the density $f_{Y_{k-1}^n, Y_k^n}$, but drop the factorials for the moment,
\begin{align*}
...
\begin{theorem}
Let $n$ jobs arrive at a station such that the arrival times are iid rvs $U_{k}\sim \Unif{[0,a]}$, then all interarrival times (the gaps) $\{X_{k}^{n}\{_{k=0}^{n+1}$ as seen by the server, i.e., as obtained from the order statistic of $\{U_{k}\}_{k=1}^{n}$, have the same distribution, i.e., $X_i^n \sim X_k^{n}$ for all $0\leq i,k \leq n$.
When the size $n$ of the population, $X_k^{n}$ is approximately $\sim \Exp{a/n}$.
\end{theorem}

There is one point that remains.
It is given that $\{U_{k}\}_{i=1}^{n}\}$ are uniform on $[0,1]$, but what is the distribution off the arrival on the next interval $[1,2]]$?
As it happens, after having discussed the next topic, this will be easy to answer.


\newthought{We next provide} the formal background for the insights of the upper part of~\cref{fig:unitoexp}.
We consider only the day that a patient visits the hospital (the time on a day is of no importance), and assume that there are $n$ days on average between two successive visits.
For example, suppose the interarrival time $X$ (in days) of a patient to visit the hospital is such that $\P{X=n-1} = \P{X=n+1}=1/2$.
Then, if the process starts on day $0$ and $X_{1} = n-1$, the next visit will be on day $n-1$, but if $X_{1}=n+1$, the next visit will be on day $1$ of the next half year.
By looking at the process like this, the patient \emph{moves on a circle} with $n$ states, and with every move the patient moves with probability $1/2$ one state to the left or to the right on the circle.

\begin{theorem}\label{thr:2}
Let $\alpha_k(i) = \P{A_k=i}$ be the probability that the $k$th arrival is on the $i$th day, where $A_{i} = (A_{i-1} + X_{i}) \bmod n$, i.e, $A_{i}$ lives on the circle space $\{0, 1, \ldots, n-1\}$.
If $\{X_{k}\}_{k=1}^{\infty}$ is a set of iid rvs such that $\E{X_{k}} < \infty$ and $\sup_{i}\P{X_{k} =i} < 1$, then, for all $i$, $\alpha_k(i) \to 1/n$ as $k\to\infty$.
\end{theorem}

\begin{proof}
We impose the condition $\E{X_{k}}<\infty$ to prevent to having to deal with $A_{k} = \infty$ for some finite $k$.
For the moment, take $\P{X_{k} = 1}=p \in (0, 1)$ and $\P{X_{k}=-1} = q=1-p$.
By first-step analysis,
\begin{equation*}
\alpha_{k+1}(i) = p \alpha_k(i-1) + q \alpha_k(i+1).
\end{equation*}
Clearly, if $\alpha_{k}(i)=1/n$ for all $i$, then $\alpha_{k+1}(i) = 1/n$ as well.

However, if not all $\alpha_{k}(\cdot)$ are equal, then as there are a finite number of states, there must be at least one state $i$ such that $\alpha_k(i-1) < \alpha_k(i) \geq \alpha_k(i+1)$.
But then, on the next step
\begin{equation*}
\alpha_{k+1}(i) = p \alpha_k(i-1) + p \alpha_k(i+1) < \alpha_k(i).
\end{equation*}
In particular, the largest $\alpha_{k}$ becomes smaller and, by symmetry, the smallest $\alpha_{k}$ becomes larger.

The argument is easy to generalize. Let $\P{X=j} = p_{j}$. By assumption $p_{j} \in (0, 1)$. Then, $\E{\alpha_k(i+X)} = \sum_{j} p_{j} \alpha_k(i+j) < \alpha_k(i)$ if $\alpha_k(i) \geq \alpha_{k}(\cdot)$ and there is at least one $j$ such that $\alpha_k(i+j) < \alpha_k(i)$ and $p_{j} > 0$.
Thus, in general, any  maximum become strictly smaller, and by the same reasoning,  any minimum becomes strictly larger.
Finally, the minimum and the maximum cannot cross after any step, because an average over a set of numbers cannot be smaller than the smallest number in that set.

All in all, this implies that $\alpha_k(i) \to 1/n$ for all $i$ as $k\to \infty$.
\end{proof}

Clearly, the proof applies to any $n=1, 2, \ldots$.
...
Suppose for some $\epsilon>0$ there is set $A_{\epsilon} = \{x \in C: f(x) \leq f(x + \bar x) -\epsilon\}$ such that $\P{A_{\epsilon}} > 0$.
(If there is no such set for any $\epsilon$, then $f$ is a constant.)
But then, $\E{f(x+X)} \leq (f(\bar x) -\epsilon) \P{A_{\epsilon}} + f(\bar x) (1-\P{A_{\epsilon}}) < f(\bar x)$.
Clearly, taking an average over $f$ makes the maximum smaller, and likewise the minimum larger.

Interstingly, the ideas of the proofs for~\cref{thr:2} are used in potential theory and complex analysis, and the limiting function distribution is known as a harmonic function.


Finally, how about the distribution of the fractional part of $Z=U+V$ when $U, V$ independent and unifom on $[0,1]]$?

\begin{theorem}
Let $Z=\sum_{i}^{n} U_{i}$ for $n$ idd rvs $U_{i}\sim \Unif{[0,1]}$, and $\lfloor x \rfloor$  the integer part of $x\in \R$.
The fractional part of $Z$ is $Z-\lfloor Z \rfloor \sim \Unif{[0,1]}$.
\end{theorem}
\begin{proof}
