...
Thus, in the rest of the book we will only study queueing systems in steady-state, by which we mean that we consider the long-run average time limits of the system, c.f., \cref{sec:rate-stability}.
...
\begin{equation}\label{eq:44}
 Z_k = Z_{k-1} + a_k - c_k.
\end{equation}
In fact, $\{Z_k\}$ is a random walk since~$Z$ makes jumps of size $a_k-c_k, k=1,\ldots$ where $\{a_k-c_k\}$ forms a a sequence of iid rvs.
