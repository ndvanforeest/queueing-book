...
\end{equation*}
and then we write $X\sim \Exp{\lambda}$. The following properties hold:\marginnote{~\cref{ex:lambda}--~\cref{ex:11}}
\begin{subequations}
  \begin{align}
...
The remarkable fact is he reverse--when -a continuous memoryless random variable is memoryless it is exponential---can also be proven, but that is harder\marginnote{\citet[Appendix 3]{yushkevich69:_markov_proces}}.

The characteristic timescales we consider in queueing theory range from minutes to a week.\marginnote{And of course there are exceptions.}
...
However, the life span of human beings is not memoryless: take~$X$ as the life span of an arbitrary person born in 1880, and $s=t = 100$ years. Then $\P{X>100}$ was small, but not zero, but $\P{X>200 \given X>100} = 0$.%\marginnote{Even if you believe that Elvis Presley is still alive.}
