...
\begin{align*}
  s_{k-1} &= \argmin\{w_{k-1}(i) : i=1,\ldots, c\},\\
...
\newthought{We now turn} to developing a simulator for the multi-server queue step by step. We organize the code by means of classes. There are many reasons for this, but one is that this allows us to represent `things' of the `real world' as code which has its own properties and behavior.

The Server is our first class.
We implement this as a dataclass because the server is a very simple class, having just two attributes: an id and a service rate so that we can allow for speed differences between servers.
We add the method \pyv{__lt__} to compare two servers based on their relative rates.
With this method we can select a free server based on our preference. Here a lower service rate means that jobs are served at a slower rate; to select a fast server over a slow server, we have to use the inequality as shown.
...
We subclass Servers from a list so that we can this list for the heapq algoritms to push and pop servers.
When a job leaves the queue, Servers will assign a server to the job based on the preferences expressed by the \pyv{__lt__} method of the Server class.
We push to the deque any server that becomes idle after finishing a job,.
Finally, we add the convenience methods \pyv{num_free} and \pyv{is_server_available} as these are easier to understand and read than their implementations in terms of \pyv{len}.
...
We implement the sojourn time and waiting time as a \pyv{property}, as this allows us to address them in the same way as, for instance, the departure time.
The \pyv{__lt__} method determines which job to select from the queue when a service can start.
As it is now, we select jobs in FIFO sequence.
If, however, we would prefer a LIFO (last in first out) queue, we only have to reverse the direction of the inequality. Another simple rule is Shortest Processing Time First. To use this,  change the inequality to this \pyv{self.load < other.load}.

There is one detail.
We might want to store jobs in a set or a dict, and this requires to be able to distinguish one job from another.
For this reason we give the option \pyv{eq=True} to the data class and include a \pyv{__hash__} method.
\inputminted[label=job.py]{python}{../code/event_stacks/job.py}

The fourth class is the Queue class.
This is also a subclass from a list so that we can use the queue as a heap in itself, and use the \pyv{__lt__} method of the Job class to select a job from the queue and send that job to the servers to be served.
\inputminted[label=queues.py]{python}{../code/event_stacks/queues.py}

An Event is just a time and job attached to it. The \pyv{__lt__} method specifies how to order events. Clearly, this must be by time.
...
The attribute \pyv{now} keeps track of the time; it points to the time of the event the simulator is currently treating.
When starting the simulation, we send a list of jobs to the simulator which then pushes an ArrivalEvent for each job to the event stack.

When a job service starts, the \pyv{serve_job} method asks for a free server, and assigns this server to the job.
...
The \pyv{run} method starts by checking whether there are still events to do, and if so, it pops an event from the event stack.
Then it retrieves the time and stores it as \pyv{now}, and gets the job attached to the event.
If the event is an ArrivalEvent, the job must have just arrived and can be served if server is free, otherwise the job is queued.
...
The code for the $M/M/c$ queue is contained in a separate file (a module) with the name \pyv{mmc.py}; you can find it on github.
...
As it turns out, our simulation supports this already, not a single line has to change. The Servers class acts as a heap, and popping occurs with the preference as expressed by the \pyv{__lt__} method of the Server class.
...
To see  what fraction that is, we use the \pyv{Counter}. After simulating, we get this.
\begin{verbatim}
...
So, let's increase the load of the hence by setting $\mu=3.1$, instead of~$4$. We get the following results.
\begin{verbatim}
...
Another interesting rule to select servers could use the cost of a server.
...
As a priority job is just a job with a priority, we can just subclass the job class from above.
...
Then we have to add a due-date attribute to jobs, and use that in the method \pyv{__lt__}  to compare jobs.
...
\item deque
\end{enumerate}
In other words Google on `realpython classes'. Then  you'll get to a great page on why classes are such a powerful idea in programming. Then Google on `realpython dataclasses', and likewise for the other concepts. BTW, you can also ask ChatGPT.
\begin{solution}
Type on the Google prompt: `realpython dataclasses', click on the link that appears, and then read and think hard.
\end{solution}
\end{exercise}

\begin{exercise}
There are some advantages to subclassing  Queue from \pyv{deque}. What are these?
\begin{solution}
