...
In this section, we develop a model\marginnote{There is not much point in combining the models of~\cref{sec:setups-batch-proc}--\cref{sec:preempt-interr-serv} into one large model.
...
Supposing that~$N$ interruptions occur during the net job service time $S_0$, the effective service time becomes\marginnote{In an insurance context, if we interpret $\{R_i\}$ as a set of claims, then $\sum_{i=1}^N R_i$ is the total claim size of~$N$ claims.
...
% More generally, if $S_0$ is also a random variable, we find\marginnote{~\cref{ex:f-3}} that $\E N =\lambda_f \E{S_0}$.

Defining the mean time to fail as $m_{f} = 1/\lambda_f$, then the  \emph{availability} is given by
\begin{equation*}
 A:=\frac{m_f}{m_f + \E R},
\end{equation*}
from which follows  easily that\marginnote{\cref{ex:80}}
...
\newthought{With some work}, we obtain\marginnote{\cref{ex:f-4}} from~\cref{eq:f1},
 \begin{align}\label{eq:f2}
\E S &= (1+\lambda_f \E R) \E{S_0}, & \V{S} &= \frac{\V{S_0}}{A^2} + \lambda_f \E{R^2} \E{S_0}.
 \end{align}
By assuming the repair times are exponentially distributed, it is possible to find a neat expression for the scv of the service times\marginnote{\cref{ex:l-161}}
