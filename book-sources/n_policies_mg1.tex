...
\newthought{The Pollaczek-Khinchine equation} can also be found as a limiting case of the expression~\cref{eq:43} of the waiting time of the $M^{B}/M/1$ queue.\sidenote{If you are not interested in maths, than you should this derivation.}
...
In fact, the proof that the expected waiting time of the $M^{B}/M/1$ converges to the Pollaczek-Khinchine equation when $\nu\to \infty$ is based on the fact that every cdf $F$, concentrated on $[0, \infty)$, can be approximated (weakly) by the sequence of cdfs\sidenote{See Asmussen, Section III.4, Applied Probability and Queues, 2003.}
\begin{equation*}
