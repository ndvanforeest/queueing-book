...
\end{equation}
where $C_a^2 = \V X/(\E X )^2$ and $C_s^2=\V S/(\E S)^2$ are the square coefficient of variation (SCV) of the inter-arrival time and service time, respectively, and the utilization of the station\marginnote{Here is a subtle point.
...
Thus, when $\rho$ must be made smaller, we have just three options.\marginnote{And not more!}
We can reduce the arrival rate $\lambda$ of jobs, for instance by blocking demand, or sending it elsewhere such as to another machine.\marginnote{And if customers have a choice, they will take their own measures, simply by going elsewhere.}
...
Finally, we can add servers, i.e., making $c$ larger.\marginnote{This is precisely what you see in supermarkets.}
...
Thus, after ensuring that~$\rho$ is sufficiently small, it becomes important to concentrate on reducing on $C_a^2$ and $C_s^2$.\marginnote{It works the other way too: systems with low variability can deal with higher load.
}
...
% as\marginnote{This can of course also be used to estimate $C_s^2$.}
% \begin{equation*}
...
% \end{equation*}
% We derive this results in steps.\marginnote{It is based an argument in~\cite{cox62:_renew_theor}.} If you don't like to extend your knowledge of statistics, you can skip this derivation.
...
% for an $\alpha$  yet to be determined,\marginnote{This is a common trick: suppose that there exists a constant to take of the scaling, and then try an extra relation to `ferret out' the scaling constant.}
% \begin{equation*}
